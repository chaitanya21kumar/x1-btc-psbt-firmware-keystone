...
import com.x1-btc-psbt-firmware.cold.protobuf.MessageProtoc;
...
public class ProtobufTest {
    ...
    public void testSync() {
        ...
        BaseProtoc.Base.Builder base = BaseProtoc.Base.newBuilder()
                .setData(payload)
                .setVersion(1)
                .setDescription("X1-BTC-PSBT-Firmware qrcode protocol")
    ...
    public void testTest(){
        String xpub = "Zpub755NbzNDFus6egJM22CLBqjabHyNV8PEWJHGBkfCSXN2W8HRPKEA2MyxvycgD4AxNMcrTuxSwKYiNJ19h9PLURGZoWyC2Tutr7SWaU1swYK";
        String zpub = "xpub661MyMwAqRbcGhu6FEm89MgNjbMJX1TjhqvN32ZNZTWXsq7kJZaSXLJ92CYMuLSUJ9ZzZ3a52bki2H1YjTZcrwtWAQDEUYDfQdqiAbUFEDY";
        byte[] xpubBytes = new B58().decodeAndCheck(xpub);
        byte[] parentFp = Arrays.copyOfRange(xpubBytes, 5, 9);
        byte[] index = Arrays.copyOfRange(xpubBytes, 9, 13);
        byte[] key = Arrays.copyOfRange(xpubBytes, 45, 78);
        byte[] chainCode = Arrays.copyOfRange(xpubBytes, 13, 45);
        System.out.println(Hex.toHexString(xpubBytes));
        System.out.println(Hex.toHexString(parentFp));
