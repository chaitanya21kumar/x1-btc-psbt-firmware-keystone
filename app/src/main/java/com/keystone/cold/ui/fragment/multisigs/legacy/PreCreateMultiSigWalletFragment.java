...
import com.x1-btc-psbt-firmware.cold.databinding.PreCreateMultisigWalletBinding;
import com.x1-btc-psbt-firmware.cold.ui.fragment.main.NumberPickerCallback;
...
public class PreCreateMultiSigWalletFragment extends MultiSigBaseFragment<PreCreateMultisigWalletBinding>
    ...
    protected void init(View view) {
        ...
        accounts = Utilities.isMainNet(mActivity) ?
                new Account[] {MultiSig.P2WSH, MultiSig.P2SH_P2WSH, MultiSig.P2SH }
                :
                new Account[] {MultiSig.P2WSH_TEST,MultiSig.P2SH_P2WSH_TEST, MultiSig.P2SH_TEST };
        account = accounts[accountValue];
        updateUI();
    }
    ...
    private void onConfirm() {
        ...
        for (int i = 1; i <= total; i++) {
            if (i == 1) {
                info.add(new CollectXpubViewModel.XpubInfo(1, multiSigViewModel.getXfp(), multiSigViewModel.getXPub(account)));
            } else {
    ...
    private void updateUI() {
        ...
        mBinding.addressType.setRemindText(multiSigViewModel.getAddressTypeString(account));
    }
